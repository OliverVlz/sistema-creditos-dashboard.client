...
import AttachedDocumentsCard, { DocumentChanges } from '../components/AttachedDocumentsCard'
...
const LoanRequestDetailPage = () => {
  ...
  const handleSendCorrection = async () => {
    if (!selectedLoanRequest || !documentChanges || documentChanges.documentsToReplace.length === 0) return
  ...
  const clientName = selectedLoanRequest?.client?.user 
    ? `${selectedLoanRequest.client.user.firstName || ''} ${selectedLoanRequest.client.user.lastName || ''}`.trim() 
    : 'Cliente'
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Cargando información de la solicitud...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {canEditDocuments() && (
            ...
            >
              <i className={`pi ${isEditingDocuments ? 'pi-times' : 'pi-pencil'}`}></i>
              ...
              {!isEditingDocuments ? (
                ...
                <div>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    Tu solicitud fue rechazada. Puedes corregir los documentos indicados y enviarla nuevamente para revisión.
                  </p>
                  <button
                ...
                <div>
                  {pendingChangesCount === 0 ? (
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                      Haz clic en el botón <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-xs">
                        <i className="pi pi-refresh"></i> Reemplazar
                      </span> en cada documento que deseas cambiar.
                    </p>
                  ) : (
                    <div className="mb-4">
                      <div className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                        ...
                        <span className="text-sm text-green-700 dark:text-green-300">
                          <strong>{pendingChangesCount}</strong> documento(s) listo(s) para enviar
                        </span>
                      </div>
                    </div>
                  )}
